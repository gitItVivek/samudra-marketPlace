...
export function SavedInterestsPage() {
  ...
  const formatNotified = (iso?: string) => {
    if (!iso) return null;
    try {
  ...
  return (
    <PageShell title="Saved interests" rightActions="none">
      <div className={styles.page}>
        <p className={styles.intro}>
          Interests from your searches and Browse. We notify you (email + Notifications tab) only when
          a matching listing is posted. Repeating the same search updates the existing interest — no
          duplicate rows.
        </p>
        {loading && <p>Loading…</p>}
              ...
              <div className={styles.itemBody}>
                ...
                {item.lastNotifiedAt && (
                  <span className={styles.meta}>
                    Last alert sent: {formatNotified(item.lastNotifiedAt)}
                  </span>
                )}
              </div>
        ...
        <Link to="/me/notifications" className={styles.back}>
          View listing alerts
        </Link>
